...
param(
    ...
    [string]$Task = "Добавь в README.md одну строку-напоминание разработчику о следующем шаге (не более 100 символов).",
...
$body = @{
    model = $MODEL
    messages = @(
        @{ role="system"; content="Отвечай одной строкой, лёгкий деловой тон. Без кавычек." },
...
try {
    ...
    Write-Host "   ⚠️ Could not read file contents" -ForegroundColor Yellow
}
...
try {
    ...
    if ($cursorResult.error) {
        Write-Host "   ⚠️ Cursor API not configured: $($cursorResult.error)" -ForegroundColor Yellow
    } else {
        Write-Host "   ✅ File opened in Cursor" -ForegroundColor Green
    }
} catch {
    Write-Host "   ⚠️ Cursor API not available" -ForegroundColor Yellow
}
